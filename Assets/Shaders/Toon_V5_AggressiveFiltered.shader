// Version 5: Aggressive Filtered Sobel Edge Detection
// Maximum noise filtering with aggressive pre-blur and hard thresholding
// Pre-Blur Radius: offset × 1.2
// Threshold Window: 0.85× to 1.15× (narrow)
// Smoothstep Passes: 4
// Power Curve: pow(edge, 3.0)

Shader "Custom/ToonShader_V5_AggressiveFiltered"
{
    Properties
    {
        [Header(Debug Mode)]
    ...
    SubShader
    {
        ...
        // MAIN TOON PASS WITH AGGRESSIVE FILTERED SOBEL
        Pass
        {
            Name "ForwardLit"
            ...
            half4 frag(v2f IN) : SV_Target
            {
                ...
                // AGGRESSIVE FILTERED SOBEL EDGE DETECTION
                ...
                if (_EnableInnerLines > 0.5)
                {
                    ...
                    // Sample 9 positions with aggressive Gaussian pre-blur
                    ...
                    edge = smoothstep(0.47, 0.53, edge); // Pass 1: Extremely tight
                    edge = smoothstep(0.35, 0.65, edge); // Pass 2
                    edge = smoothstep(0.25, 0.75, edge); // Pass 3
                    edge = smoothstep(0.15, 0.85, edge); // Pass 4: Gradual transition

                    // Maximum power curve (3.0) for ultimate noise suppression
                    edge = pow(edge, 3.0);
