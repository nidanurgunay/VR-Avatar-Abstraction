// Version 3 Separable: Light Filtered Sobel with Separable Gaussian Blur
// OPTIMIZED: Uses separable 2-pass blur (5+5=10 samples vs 25 samples = 60% faster)
// Pre-Blur Radius: offset × 0.5
// Threshold Window: 0.3× to 2.5× (wide)
// Smoothstep Passes: 1
// Power Curve: pow(edge, 1.2)

Shader "Custom/ToonShader_V3_Separable"
{
    Properties
    {
        [Header(Debug Mode)]
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Gaussian weights for 5-tap 1D kernel (sigma ~1.0)
            // Weights: [0.0625, 0.25, 0.375, 0.25, 0.0625] normalized
            ...
            float SampleLuminanceBlurH(float2 uv, float blurRadius)
            {
                float lum = 0.0;
                ...
                for (int i = 0; i < 5; i++)
                {
                    ...
                    lum += dot(SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, uv + offset).rgb, lumCoeff) * gaussWeights[i];
                }
                return lum;
            }

            // SEPARABLE GAUSSIAN BLUR - Vertical pass (applied to horizontal results)
            // For single-pass approximation, we sample in a cross pattern
            float SampleLuminanceSeparable(float2 uv, float blurRadius)
            {
                float lum = 0.0;
                float3 lumCoeff = float3(0.299, 0.587, 0.114);

                // Horizontal samples (5 taps)
                float hSum = 0.0;
                [unroll]
                for (int i = 0; i < 5; i++)
                {
                ...
                // Vertical samples (5 taps, but center already counted in horizontal)
                float vSum = 0.0;
                [unroll]
                for (int j = 0; j < 5; j++)
                {
            ...
            half4 frag(v2f IN) : SV_Target
            {
                ...
                // SEPARABLE GAUSSIAN SOBEL EDGE DETECTION
                float edgeStrength = 0.0;
                if (_EnableInnerLines > 0.5)
                {
                    float offset = _InnerLineBlur * 0.001;
                    float blurOffset = offset * 0.5; // Light blur radius (0.5×)
                    ...
                    // Light filtering: wide threshold window (0.3× to 2.5×)
                    float minEdge = _InnerLineThreshold * 0.3;
                    float maxEdge = _InnerLineThreshold * 2.5;
                    float edge = smoothstep(minEdge, maxEdge, edgeMagnitude);

                    // Single smoothstep + light power curve (1.2)
                    edge = pow(edge, 1.2);
